...
public static final String ANSI_WHITE = "\u001B[37m";
    ...
    public static void impM(int[][] MTr)//imprime la matriz
    {
        ...
        for(int i = 0; i < MTr.length; i++){
            ...
            for(int j=0;j<MTr[0].length;j++){
               System.out.print(ANSI_GREEN+MTr[i][j]+" "+ANSI_RESET);
            }
            System.out.println();
        }
    }
     public static void impMRESULTADO(int[][] MTr)//imprime la matriz
    {
        System.out.println("\t\t\tMATRIZ - MEJOR RUTA:\n");
        for(int i = 0; i < MTr.length; i++){
            System.out.print("\t");
            for(int j=0;j<MTr[0].length;j++){
    ...
    public static boolean buscar(int x, int y)//imprime la matriz
    {
         boolean b=false;
         int c=0; 
         
         while(c<Pila.size()){
         if(Pila.get(c).Igual(x, y)){ b=true; }
         c++; }
         return b;
    }
    ...
    while(i<Pila.size()){  System.out.print("     "+(i+1)+" "); Pila.get(i).impP(); i++; } }
